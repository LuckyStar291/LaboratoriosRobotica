...
def rot_y(angle): #matriz 3x3 respecto al eje Y
    ...
    return np.array([
        [ np.cos(angle), 0, np.sin(angle), T_t1],
        [             0, 1,             0, T_t2],
        [-np.sin(angle), 0, np.cos(angle), T_t3],
        [             0, 0,             0,    1]
    ])
...
def rot_x(angle): #matriz 3x3 respecto al eje X
    ...
    return np.array([
        [1,             0,              0, T_t1],
        [0, np.cos(angle), -np.sin(angle), T_t2],
        [0, np.sin(angle),  np.cos(angle), T_t3],
        [             0, 0,             0,    1]
    ])
def rot_z(angle): #matriz 3x3 respecto al eje Z
    angle = np.deg2rad(angle)
    return np.array([
        [np.cos(angle), -np.sin(angle), 0, T_t1],
        [np.sin(angle),  np.cos(angle), 0, T_t2],
        [            0,              0, 1, T_t3],
        [             0, 0,             0,    1]
    ])
