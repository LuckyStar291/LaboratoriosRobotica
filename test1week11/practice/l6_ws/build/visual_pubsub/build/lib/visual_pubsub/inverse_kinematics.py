...
class InverseKinematics(Node):
    ...
    def target_callback(self, msg): # callback para actualizar la posición objetivo xyz
        self.target_pos = np.array([msg.x, msg.y, msg.z])
        self.get_logger().info(
            f"New target received: [{msg.x}, {msg.y}, {msg.z}]")

    def update_joints(self): #implementar el metodo analitico jacobiano para calcular la cinemática inversa
        ...
        if error_norm > self.tolerance:
            ...
            JtJ = J.T @ J #pseudo-inverse with damping
