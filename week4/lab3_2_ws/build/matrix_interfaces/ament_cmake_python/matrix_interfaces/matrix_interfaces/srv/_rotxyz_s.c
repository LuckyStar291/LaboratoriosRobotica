// generated from rosidl_generator_py/resource/_idl_support.c.em
// with input from matrix_interfaces:srv/Rotxyz.idl
// generated code does not contain a copyright notice
#define NPY_NO_DEPRECATED_API NPY_1_7_API_VERSION
#include <Python.h>
#include <stdbool.h>
#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-function"
#endif
#include "numpy/ndarrayobject.h"
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif
#include "rosidl_runtime_c/visibility_control.h"
#include "matrix_interfaces/srv/detail/rotxyz__struct.h"
#include "matrix_interfaces/srv/detail/rotxyz__functions.h"


ROSIDL_GENERATOR_C_EXPORT
bool matrix_interfaces__srv__rotxyz__request__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    char full_classname_dest[45];
    {
      char * class_name = NULL;
      char * module_name = NULL;
      {
        PyObject * class_attr = PyObject_GetAttrString(_pymsg, "__class__");
        if (class_attr) {
          PyObject * name_attr = PyObject_GetAttrString(class_attr, "__name__");
          if (name_attr) {
            class_name = (char *)PyUnicode_1BYTE_DATA(name_attr);
            Py_DECREF(name_attr);
          }
          PyObject * module_attr = PyObject_GetAttrString(class_attr, "__module__");
          if (module_attr) {
            module_name = (char *)PyUnicode_1BYTE_DATA(module_attr);
            Py_DECREF(module_attr);
          }
          Py_DECREF(class_attr);
        }
      }
      if (!class_name || !module_name) {
        return false;
      }
      snprintf(full_classname_dest, sizeof(full_classname_dest), "%s.%s", module_name, class_name);
    }
    assert(strncmp("matrix_interfaces.srv._rotxyz.Rotxyz_Request", full_classname_dest, 44) == 0);
  }
  matrix_interfaces__srv__Rotxyz_Request * ros_message = _ros_message;
  {  // px
    PyObject * field = PyObject_GetAttrString(_pymsg, "px");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->px = PyLong_AsLongLong(field);
    Py_DECREF(field);
  }
  {  // py
    PyObject * field = PyObject_GetAttrString(_pymsg, "py");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->py = PyLong_AsLongLong(field);
    Py_DECREF(field);
  }
  {  // pz
    PyObject * field = PyObject_GetAttrString(_pymsg, "pz");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->pz = PyLong_AsLongLong(field);
    Py_DECREF(field);
  }

  return true;
}

ROSIDL_GENERATOR_C_EXPORT
PyObject * matrix_interfaces__srv__rotxyz__request__convert_to_py(void * raw_ros_message)
{
  /* NOTE(esteve): Call constructor of Rotxyz_Request */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("matrix_interfaces.srv._rotxyz");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Rotxyz_Request");
    assert(pymessage_class);
    Py_DECREF(pymessage_module);
    _pymessage = PyObject_CallObject(pymessage_class, NULL);
    Py_DECREF(pymessage_class);
    if (!_pymessage) {
      return NULL;
    }
  }
  matrix_interfaces__srv__Rotxyz_Request * ros_message = (matrix_interfaces__srv__Rotxyz_Request *)raw_ros_message;
  {  // px
    PyObject * field = NULL;
    field = PyLong_FromLongLong(ros_message->px);
    {
      int rc = PyObject_SetAttrString(_pymessage, "px", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // py
    PyObject * field = NULL;
    field = PyLong_FromLongLong(ros_message->py);
    {
      int rc = PyObject_SetAttrString(_pymessage, "py", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // pz
    PyObject * field = NULL;
    field = PyLong_FromLongLong(ros_message->pz);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pz", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }

  // ownership of _pymessage is transferred to the caller
  return _pymessage;
}

#define NPY_NO_DEPRECATED_API NPY_1_7_API_VERSION
// already included above
// #include <Python.h>
// already included above
// #include <stdbool.h>
// already included above
// #include "numpy/ndarrayobject.h"
// already included above
// #include "rosidl_runtime_c/visibility_control.h"
// already included above
// #include "matrix_interfaces/srv/detail/rotxyz__struct.h"
// already included above
// #include "matrix_interfaces/srv/detail/rotxyz__functions.h"


ROSIDL_GENERATOR_C_EXPORT
bool matrix_interfaces__srv__rotxyz__response__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    char full_classname_dest[46];
    {
      char * class_name = NULL;
      char * module_name = NULL;
      {
        PyObject * class_attr = PyObject_GetAttrString(_pymsg, "__class__");
        if (class_attr) {
          PyObject * name_attr = PyObject_GetAttrString(class_attr, "__name__");
          if (name_attr) {
            class_name = (char *)PyUnicode_1BYTE_DATA(name_attr);
            Py_DECREF(name_attr);
          }
          PyObject * module_attr = PyObject_GetAttrString(class_attr, "__module__");
          if (module_attr) {
            module_name = (char *)PyUnicode_1BYTE_DATA(module_attr);
            Py_DECREF(module_attr);
          }
          Py_DECREF(class_attr);
        }
      }
      if (!class_name || !module_name) {
        return false;
      }
      snprintf(full_classname_dest, sizeof(full_classname_dest), "%s.%s", module_name, class_name);
    }
    assert(strncmp("matrix_interfaces.srv._rotxyz.Rotxyz_Response", full_classname_dest, 45) == 0);
  }
  matrix_interfaces__srv__Rotxyz_Response * ros_message = _ros_message;
  {  // pa
    PyObject * field = PyObject_GetAttrString(_pymsg, "pa");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->pa = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // pb
    PyObject * field = PyObject_GetAttrString(_pymsg, "pb");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->pb = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // pc
    PyObject * field = PyObject_GetAttrString(_pymsg, "pc");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->pc = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }

  return true;
}

ROSIDL_GENERATOR_C_EXPORT
PyObject * matrix_interfaces__srv__rotxyz__response__convert_to_py(void * raw_ros_message)
{
  /* NOTE(esteve): Call constructor of Rotxyz_Response */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("matrix_interfaces.srv._rotxyz");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Rotxyz_Response");
    assert(pymessage_class);
    Py_DECREF(pymessage_module);
    _pymessage = PyObject_CallObject(pymessage_class, NULL);
    Py_DECREF(pymessage_class);
    if (!_pymessage) {
      return NULL;
    }
  }
  matrix_interfaces__srv__Rotxyz_Response * ros_message = (matrix_interfaces__srv__Rotxyz_Response *)raw_ros_message;
  {  // pa
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->pa);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pa", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // pb
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->pb);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pb", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // pc
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->pc);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pc", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }

  // ownership of _pymessage is transferred to the caller
  return _pymessage;
}
